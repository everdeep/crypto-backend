import os
import requests
from api.model import UserModel, UserSettingsModel, ServerActivityModel, AddressModel, PortfolioModel

from api.service.email import EmailService
from api.enums import UserActions
...
class UserService:
    ...
    def get_activity(self, user_id, action=None):
        if action:
            try:
                ...
                return ServerActivityModel.query.filter_by(user_id=user_id, endpoint=endpoint).order_by(ServerActivityModel.id.desc()).all()
            except KeyError:
                return None
        
        return ServerActivityModel.query.filter_by(user_id=user_id).order_by(ServerActivityModel.id.desc()).all()
    ...
    def update_user_details(self, user_id, user_details):
        user = UserModel.query.filter_by(id=user_id).first()
        if user:
            user.first_name = user_details.get("firstName") if user_details.get("firstName") else user.first_name
            user.last_name = user_details.get("lastName") if user_details.get("lastName") else user.last_name
            user.email = user_details.get("email") if user_details.get("email") else user.email
            user.username = user_details.get("username") if user_details.get("username") else user.username
            user.phone = user_details.get("phoneNumber") if user_details.get("phoneNumber") else user.phone
    ...
    def register(self, user_details, authType="local"):
        ...
        EmailService().send_email("Welcome to Mimic", template, recipients=[user.email], context=context)
