...
def test_role_schema(session):
    """Test the role schema"""
    data = session.query(RoleModel).first()
    data = RoleSchema().dump(data)
    assert "id" in data
    assert "name" in data
...
def test_server_activity_schema(session):
    """Test the server_activity schema"""
    data = session.query(ServerActivityModel).first()
    if data is None:
        return
    data = ServerActivitySchema().dump(data)
    assert "id" in data
    assert "url" in data
    assert "ua_browser" in data
    assert "ua_language" in data
    assert "ua_platform" in data
    assert "ua_version" in data
    assert "blueprint" in data
    assert "view_args" in data
    assert "status" in data
    assert "remote_addr" in data
    assert "xforwardedfor" in data
